...
def replace_struct_in_query(query):
    ...
    pattern = re.compile(
        r"Struct\s*\(\s*Struct\s*\(\s*([^\(\)]+)\s*\)\s*\)", re.IGNORECASE
    )
...
async def convert_query(
    ...
    try:
        ...
        converted_query = sqlglot.transpile(
            query, read=from_sql, write=to_sql, identify=False
        )[0]
        ...
        double_quotes_added_query = quote_identifiers(
            converted_query_ast, dialect=to_sql
        ).sql(dialect=to_sql)
...
if __name__ == "__main__":
    uvicorn.run(
        "converter_api:app", host="0.0.0.0", port=8100, proxy_headers=True, workers=5
    )
