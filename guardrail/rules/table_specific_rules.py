...
class RequiredColumnRule(Rule):
    ...
    def validate(self, query: Dict[str, Any], table_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if table in self.applicable_tables:
            ...
            if "*" in columns:
                logging.debug(
                    f"Wildcard '*' detected in Query for table '{table}'. No violation for required columns."
                )
                return violations
    ...
    def validate(self, query: Dict[str, Any], table_info: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if table in self.applicable_tables:
            ...
            if "*" in columns:
                ...
                logging.debug(
                    f"ForbiddenColumnRule triggered by wildcard '*' in Query for table '{table}'."
                )
                return violations
            ...
            if used_forbidden:
                ...
                logging.debug(
                    f"ForbiddenColumnRule triggered by columns {used_forbidden} in Query for table '{table}'."
                )
        return violations
