    ...
    for rule_def in raw_rules:
        ...
        if scope == 'global':
            ...
            if rule_type == 'required_column':
                ...
                logging.warning(f"Unknown table-specific rule type: {rule_type}. Skipping rule ID: {rule_def['id']}")
                continue
        else:
...
def validate_queries_dynamic(queries: List[Dict[str, Any]], table_map: Dict[str, Any], rules: List[Any]) -> List[Dict[str, Any]]:
    violations = []
