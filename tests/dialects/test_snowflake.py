  ...
  file_format = (type = parquet)""",
    ...
    def test_table_literal(self):
        ...
        self.validate_all(
            ...
            write={
                "snowflake": r"""SELECT * FROM TABLE('MYDB."MYSCHEMA"."MYTABLE"')"""
            },
        )
...
STORAGE_ALLOWED_LOCATIONS=('s3://mybucket1/path1/', 's3://mybucket2/path2/')""",
    ...
    def test_try_cast(self):
        ...
        self.assertEqual(
            expression.sql(dialect="snowflake"), "SELECT TRY_CAST(t.x AS TEXT) FROM t"
        )

        expression = annotate_types(expression, schema={"t": {"x": "int"}})
        self.assertEqual(
            expression.sql(dialect="snowflake"), "SELECT CAST(t.x AS TEXT) FROM t"
        )
...
SINGLE = TRUE""",
    ...
    def test_bit_or_function(self):
        ...
        self.validate_identity(
            "SELECT BIT_OR(a, b) FROM table", "SELECT BITOR(a, b) FROM table"
        )
    ...
    def test_bit_not_function(self):
        self.validate_identity(
            "SELECT BIT_NOT(a) FROM table", "SELECT BITNOT(a) FROM table"
        )

        self.validate_identity(
            "SELECT BIT_NOT(-1) FROM table", "SELECT BITNOT(-1) FROM table"
        )
