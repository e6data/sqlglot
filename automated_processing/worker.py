...
def process_query_batch(self, job_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (
        ...
        and self.request
        and hasattr(self.request, "hostname")
        and hasattr(self.request, "id")
    ):
    ...
    try:
        ...
        for chunk_start in tqdm(
            range(0, num_queries, chunk_size), desc=progress_desc, unit="chunks"
        ):
            ...
            chunk_results = pa.table(
                {
                    ...
                    "supported_functions": pa.array(
                        supported_functions_lists, type=pa.list_(pa.string())
                    ),
                    "unsupported_functions": pa.array(
                        unsupported_functions_lists, type=pa.list_(pa.string())
                    ),
                    "unsupported_functions_after_transpilation": pa.array(
                        unsupported_functions_after_transpilation_lists, type=pa.list_(pa.string())
                    ),
        ...
        try:
            ...
            logger.error(
                f"❌ Critical Iceberg error - continuing without storage: {str(iceberg_error)}"
            )
