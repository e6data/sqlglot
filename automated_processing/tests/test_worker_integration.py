...
class TestWorkerIntegration:
    ...
    def assert_query_result(self, expected, actual, query_index):
        ...
        expected_functions = (
            set(expected["expected_supported_functions"])
            if isinstance(expected["expected_supported_functions"], list)
            else set()
        )
        actual_functions = (
