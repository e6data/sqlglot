...
def run_integration_test():
    ...
    try:
        ...
        assert len(result['query_results']) == 500, f"Expected 500 results, got {len(result['query_results'])}"
        ...
        for i, (expected, actual) in enumerate(zip(expected_results, result['query_results'])):
            try:
                ...
                expected_funcs = set(expected['supported_functions']) if isinstance(expected['supported_functions'], list) else set()
                actual_funcs = set(actual['supported_functions']) if isinstance(actual['supported_functions'], list) else set()
