...
def orchestrate_processing(
    ...
    try:
        ...
        file_paths = discover_parquet_files(directory_path, query_column)
        ...
        for file_path in file_paths:
            ...
            unique_table = extract_unique_queries_from_file(file_path, query_column, filters or {})
        ...
        if not all_batch_configs:
            return {"error": "No valid queries found in any files", "session_id": session_id}
        ...
        return {
            ...
            "task_ids": [
                f"{session_id}_batch_{config['batch_id']}" for config in all_batch_configs
            ],
        ...
        return {"error": str(e), "session_id": session_id, "status": "failed"}
...
def get_processing_status(session_id: str, task_ids: List[str] = None) -> Dict[str, Any]:
    ...
    for task_id in task_ids:
        ...
        task_status = {"task_id": task_id, "state": result.state}
    ...
    try:
        ...
        if (
            (completed + failed >= actual_total_batches)
            and session_start_time
            and not session_end_time
        ):
    ...
    progress_percentage = (
        (successful_batches / actual_total_batches * 100) if actual_total_batches > 0 else 0
    )
    ...
    return {
        ...
        if completed + failed >= actual_total_batches
        else "processing",
